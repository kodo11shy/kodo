# Codex 与 Claude Code 协作联系单（历史归档）

> **📦 本文件已于 2026-06-18 归档，不再作为当前协作主文件。**
>
> 当前最新工作联系单为：
...
## Round 88：域名备案通过，等待 Hermes 部署后端

**时间**：2026-06-11
**状态**：ccrong.cloud 已备案通过，域名可访问（当前为占位页）
**config.js**：已切回生产模式 `https://ccrong.cloud/api`，已推送

### Hermes 需执行

1. 在服务器 `git pull` 获取最新代码（含生产配置）
2. 按 Round 87 部署步骤启动后端
3. 确认 `https://ccrong.cloud/api/` 返回 200
4. 确认 HTTPS 证书生效
5. 通知用户验收
...
## Codex补充记录 - 2026-06-18 体验版优化与收口阶段

收到用户提供的 GPT 综合指令后，当前阶段切换为：
...
1. 不继续扩展新功能。
2. 优先处理 P0/P1：打不开、接口失败、数据错乱、流程不顺、关键按钮不明显、表单不好填写。
3. P2 只做低风险视觉/文案收口。
4. P3 新功能建议只记录，暂不开发。
...
```text
miniprogram/config.js
https://ccrong.cloud/api
```

2. 生产后端健康检查已通过：

```text
https://ccrong.cloud/api/health
status = ok
env = production
database.ok = true
...
```text
miniprogram/project.config.json
appid = wxe1884374620755fb
miniprogramRoot = .
urlCheck = true
```

注意：仓库根目录也存在 `project.config.json`，且 appid 与 `miniprogram/project.config.json` 不一致。体验版操作仍应打开：

```text
E:\projects\托班智慧管理系统开发\miniprogram
```

不要打开仓库根目录。是否删除或同步根目录项目配置，需用户/Claude Code 再确认。
...
后续老师/家长/用户反馈统一追加到该文件，不再散落在聊天记录里。
...
```text
app.json 页面注册检查：25 个页面均有 js/json/wxml/wxss
小程序 JS 语法检查：29 个 JS 文件通过
小程序 JSON 解析：29 个 JSON 文件通过
后端 Python 编译：backend/app 通过
图片资源引用检查：/images 引用均存在
页面跳转静态检查：跳转目标均在 app.json 注册页面内
生产 API 健康检查：通过
git diff --check：通过
```

说明：无法在当前自动化环境中真正打开微信开发者工具逐页肉眼验收，因此“所有页面能否在开发者工具中正常打开”和“真机视觉效果”仍需人工在微信开发者工具中确认。

### 本轮已做的体验收口修改

重点修复所有用户截图暴露出的“表单字段散到遮罩外/弹窗不像正式产品”的问题，并对同类输入弹窗做了全局收口。
...
```text
miniprogram/pages/teacher/notices/notices.wxml
miniprogram/pages/teacher/notices/notices.wxss
miniprogram/pages/teacher/meal/meal.wxml
miniprogram/pages/teacher/meal/meal.wxss
miniprogram/pages/teacher/payment/payment.wxml
miniprogram/pages/teacher/payment/payment.wxss
miniprogram/pages/teacher/settings/settings.wxml
miniprogram/pages/teacher/settings/settings.wxss
miniprogram/pages/teacher/admin/admin.js
miniprogram/pages/teacher/admin/admin.wxml
miniprogram/pages/teacher/admin/admin.wxss
miniprogram/pages/teacher/student-list/student-list.wxml
miniprogram/pages/teacher/student-list/student-list.wxss
miniprogram/pages/teacher/student-detail/student-detail.wxml
miniprogram/pages/teacher/student-detail/student-detail.wxss
miniprogram/pages/teacher/growth/growth.wxml
miniprogram/pages/teacher/growth/growth.wxss
miniprogram/pages/teacher/attendance/attendance.wxml
miniprogram/pages/teacher/attendance/attendance.wxss
```

处理方式：

1. 将高风险表单弹窗从公共 `modal-overlay / modal-content / input-field / form-group` 样式中拆出，改成各页面专用 `*-sheet`、`*-input`、`*-field`。
2. 长表单统一为“固定头部 + 可滚动内容 + 固定底部按钮”。
3. 老师账号编辑中，管理员不再显示“学科：不限（管理员）”；只有老师角色才显示语文/数学学科。
4. 收费管理顶部“+ 记一笔”按钮改为 flex 居中。

### 当前已发现但未直接修改的问题

1. **P1 配置风险**：仓库根目录 `project.config.json` 与 `miniprogram/project.config.json` appid 不一致。建议用户只打开 `miniprogram/`；是否同步根目录配置需确认。
2. **P2 文案/配置风险**：关于页隐私政策、用户协议仍指向 `tuoban.example.com` 占位链接。需要用户提供正式链接。
3. **P0 人工确认项**：微信公众平台合法域名需确认包含：

```text
request 合法域名：https://ccrong.cloud
uploadFile 合法域名：https://ccrong.cloud
downloadFile 合法域名：https://ccrong.cloud
```

4. **P1 正式上线前配置项**：真实微信登录仍需 AppID/AppSecret 和 `WECHAT_MOCK_LOGIN=false` 联调；体验版当前可继续测试。

### 给 Claude Code 的前端处理建议

若用户后续继续交给 Claude Code 做前端，请按以下优先级：

1. **P1**：用微信开发者工具逐页打开并截图核对本轮所有表单弹窗，重点看小屏 iPhone 视口。
2. **P1**：确认根目录 `project.config.json` 是否需要删除、同步或标注，避免误打开错误项目。
3. **P2**：关于页隐私政策/用户协议替换为用户确认的正式链接。
4. **P2**：作业创建、作业批改两个完整页面表单做真机视觉检查；当前静态检查未发现同类弹窗散开问题。
...
微信公众平台合法域名是否配置
是否提供正式隐私政策/用户协议链接
是否提供真实微信 AppID/AppSecret 并关闭 mock
体验版老师测试名单和真实反馈
是否处理仓库根目录 project.config.json 的 appid 不一致问题
```

---

## Codex补充记录 - 2026-06-17 收费管理弹窗二次修正

用户补充截图显示老师端“收费管理 -> 记一笔”仍然出现表单项散到灰色遮罩上的问题。已对该弹窗做二次加固：
...
1. 不再使用通用 `modal-overlay / modal-content / modal-actions` 组合，改为独立 `payment-modal-overlay / payment-sheet / payment-sheet-actions`。
2. 弹窗内部固定为“头部 + 可滚动表单区 + 底部按钮区”，避免输入框、picker、按钮脱离白色面板。
3. 开始日期、结束日期继续使用 `picker mode="date"`，避免手输日期造成移动端排版和输入体验问题。
4. 输入框和 picker 样式限定在 `.payment-sheet` 内，降低与页面其他收费卡片、筛选区样式互相影响的风险。

验收重点：重新编译小程序后打开 `老师端 -> 收费管理 -> 记一笔`，确认所有字段都在白色弹窗面板内，底部“取消/保存”固定在弹窗底部。

---

## Codex补充记录 - 2026-06-17 通知发布弹窗二次修正

用户再次截图确认 `老师端 -> 通知管理 -> 发布通知` 仍有字段散到遮罩外的问题。已对通知发布/编辑弹窗做同样的二次加固：
...
1. 发布/编辑通知弹窗不再使用公共 `modal-overlay / modal-content / modal-actions`，改为独立 `notice-sheet-overlay / notice-sheet / notice-sheet-actions`。
2. 表单内部也不再使用公共 `input-field / picker-field / form-group / form-row`，全部替换为 `notice-control / notice-picker / notice-field / notice-row`。
3. 保留日期选择器，并将弹窗固定为“头部 + 滚动表单区 + 底部按钮区”，避免标题、内容、类型、日期等字段散落在灰色遮罩上。

验收重点：重新编译小程序后打开 `老师端 -> 通知管理 -> 发布通知`，确认白色弹窗完整包住标题、内容、类型、置顶、日期和底部按钮。

---

### Codex 留言

2026-06-11（托班系统上线测试推进指令）：

我已读取 Hermes 的部署扫描报告。当前结论如下，请 Hermes / Claude Code 按这个方向继续，不要再把问题理解成“缺 Web 管理后台”。
...
```text
尽快把托班智慧管理系统上线为可测试版本，交给老师实际使用并反馈问题。
```

一期交付形态是：

```text
微信小程序前端 + ccrong.cloud 生产后端 API
```

`/var/www/ccrong/` 只有占位页不是当前阻塞项。Web 官网/后台可以后续再做，一期先让老师通过微信小程序使用。

**二、当前状态判断**

Hermes 已完成/确认：

- 后端 API 已部署到 `https://ccrong.cloud/api/`
- PostgreSQL 已连接
- HTTPS 证书已生效
- nginx 反向代理正常
- 上传路径代理正常
...
**三、Claude Code / Codex 负责事项**

请 Claude Code / Codex 处理小程序前端上线测试：
...
```text
E:\projects\托班智慧管理系统开发\miniprogram\config.js
```
...
请 Hermes 继续负责生产后端和服务器安全收口：

1. 保持 `https://ccrong.cloud/api/` 可用。
2. 确认健康检查：
...
```env
TOKEN_SECRET=使用 openssl rand -hex 32 生成的新密钥
DEFAULT_TEACHER_PASSWORD=改成临时强密码，并告知用户
```

4. 微信登录暂时可以保持 mock，便于体验版先测试：

```env
WECHAT_MOCK_LOGIN=true
```

等用户提供小程序 `AppID` / `AppSecret` 后，再改为：

```env
WECHAT_APP_ID=真实值
WECHAT_APP_SECRET=真实值
WECHAT_MOCK_LOGIN=false
```

5. 若修改 `.env`，请重启后端服务并回报结果：
...
- 微信公众平台是否已配置合法域名：`https://ccrong.cloud`
- 是否能提供小程序 AppID / AppSecret
- 老师体验版测试名单
- 老师测试后反馈的问题
...
1. P0：小程序连接生产 API，上传体验版
2. P0：修改默认密码和 TOKEN_SECRET
3. P0：给老师测试并收集问题
4. P1：修复老师测试中发现的问题
5. P1：接入正式微信登录
6. P2：Web 占位页/官网/管理后台，后续再评估

请后续所有 AI 围绕“上线给老师试用”推进，不要扩散到新功能或 Web 后台建设。
...
2026-06-11（本地小程序接生产 API 处理进展）：

已收到 Hermes 回复：生产后端 `https://ccrong.cloud/api/` 运行中，健康检查、PostgreSQL、uploads、TOKEN_SECRET、默认密码均已由 Hermes 处理完成，微信登录暂时保持 mock。
...
3. 已扫描小程序目录，没有发现 `localhost`、`127.0.0.1`、`192.168.*`、`8001` 等本地 API 残留。

4. 已清理家长端登录页的开发测试入口：

- 删除“开发环境 OpenID”输入框
- 删除“测试进入”按钮
- 家长绑定改为正式路径：`wx.login` → `/auth/wechat/session` → `/auth/parent/bind`
- 当前 Hermes 保持 `WECHAT_MOCK_LOGIN=true` 时也可正常返回 mock openid，适合体验版测试

5. 已移除后端未提交的 `/auth/dev/parent-login` 开发测试接口，避免体验版误用测试入口。

6. 已做本地静态校验：

```text
node --check parent/login/login.js 通过
python -m py_compile backend/app/api/routes/auth.py 通过
硬编码扫描通过
```

**当前剩余动作**

请 Claude Code / 本地操作者用微信开发者工具继续：
...
2. 老师体验版测试名单是谁。
3. 是否现在就提供小程序 AppID / AppSecret。若暂时不提供，体验版可继续使用 mock 登录测试；正式上线前再关闭 mock。

**注意**

当前工作区还有一批未提交改动，其中包括其他 AI/用户此前对家长端作业页、老师端作业列表页等文件的修改；我没有回滚这些改动，只处理了上线前明显不该暴露的家长端开发入口。

---

### Codex 留言

2026-06-17（接上上线验收前体检与小修）：

我已按当前主线继续推进，目标仍然是“微信小程序体验版接生产 API，尽快交给老师试用”，没有扩散到 Web 后台或新模块。
...
3. 已扫描小程序和后端目录，没有发现体验版不该保留的 `auth/dev`、`dev/parent-login`、`开发环境OpenID`、`测试进入`、`localhost:8001` 等命中。
...
此前 `page_size` 被改为 `200`，但后端 `/api/homework` 限制最大 `100`，生产环境会返回 422；现已改回 `100`。

2. 修复家长端作业页照片循环变量遮蔽问题：

```text
miniprogram/pages/parent/homework/homework.wxml
```

三组照片循环已显式使用 `photo` 变量，避免内层 `item` 遮蔽外层作业记录，导致预览/长按保存照片时取错数据。

**已完成校验**

```text
node --check miniprogram/**/*.js 通过
miniprogram/**/*.json JSON 解析通过
...
2. 老师体验版测试名单。
3. 是否现在提供小程序 AppID / AppSecret；若暂不提供，体验版继续使用 mock 登录，正式上线前再关闭 mock。

---

### Codex 留言

2026-06-17（老师/家长微信登录与免重复输入策略）：

本轮按用户要求补齐登录体验：

**已确认的原状态**

1. 家长端“第一次邀请码绑定微信”已经有后端和前端主流程：`wx.login` → `/auth/wechat/session` → `/auth/parent/bind`。
2. 后端已有老师绑定微信接口 `/api/auth/teacher/bind-wechat`，但此前小程序没有入口，也没有老师微信直接登录接口。
3. 老师端此前在 `app.js` 里每次启动都会清掉老师登录态，所以必须重复输入账号密码。
...
```text
teacher_login_mode = always / duration / remember
teacher_login_remember_hours = 1-720
```
...
绑定过微信的老师可直接登录；未绑定会提示先用账号密码登录后绑定。
...
```text
always   = 每次都输入账号密码
duration = 指定小时内免重复输入，默认 2 小时，可在后台调成 1 小时等
remember = 登录一次后长期记住本机，手动退出后失效
```

5. 老师登录页新增“微信登录”按钮，并显示当前登录保持策略。
6. 系统设置页新增“登录设置”：

```text
老师登录方式：每次都输入 / 限时免输入 / 长期记住
免输时长：小时数，可调
老师微信登录：绑定/重新绑定当前微信
```
...
```text
已绑定过邀请码的家长，再次进入家长入口时，会用当前微信尝试 /auth/parent/auto-login
未绑定过的家长继续停留在邀请码输入页
```
...
backend/app/db/init_db.py
miniprogram/app.js
miniprogram/pages/teacher/login/login.js
miniprogram/pages/teacher/login/login.wxml
miniprogram/pages/teacher/login/login.wxss
miniprogram/pages/teacher/settings/settings.js
miniprogram/pages/teacher/settings/settings.wxml
miniprogram/pages/teacher/settings/settings.wxss
miniprogram/pages/parent/login/login.js
miniprogram/pages/parent/login/login.wxml
```
...
python -m compileall backend/app 通过
git diff --check 通过
开发入口扫描无 auth/dev、dev/parent-login、开发环境OpenID、测试进入
```

**联调注意**

1. 生产后端部署本次代码后需要重启，让 `seed_default_config` 补入新配置项。
2. 体验版当前如果 `WECHAT_MOCK_LOGIN=true`，微信登录/绑定会使用小程序本机缓存的稳定 mock openid，可用于测试“绑定一次、下次自动登录/微信登录”；正式上线前再切真实 AppID/AppSecret 并关闭 mock。
3. 老师微信登录必须先用账号密码登录一次，在系统设置里绑定当前微信后，后续才能点“微信登录”。

---

### Codex 留言

2026-06-17（通知/餐食/收费弹窗布局修复）：

用户反馈三个 UI 问题：

1. 公开首页通知详情内容横向溢出到屏幕外。
2. 老师端“发布通知”表单字段散开，没有正确收在弹窗面板内。
3. 老师端“记录餐食”和“添加收费项目/记一笔”弹窗也存在类似布局不稳。
...
弹窗改为 `86vw` 响应式宽度，内容区强制 `pre-wrap + break-all`，避免长通知内容冲出屏幕。

2. 家长首页通知详情：

```text
miniprogram/pages/parent/dashboard/dashboard.wxss
```

同步修复通知内容换行和弹窗宽度，避免家长端出现同样问题。
...
miniprogram/pages/teacher/notices/notices.wxml
miniprogram/pages/teacher/notices/notices.wxss
miniprogram/pages/teacher/notices/notices.js
```

表单改成独立弹窗面板，带固定头部、滚动内容区和底部操作区；开始/结束显示改成日期选择器，并新增“设为长期”。
...
餐食表单改成居中大面板，避免字段散在灰色遮罩上；日期改成日期选择器。
...
添加/编辑收费项目弹窗改成稳定面板，输入框固定高度，金额/单位两栏不互相挤，按钮固定在底部。
...
## 2026-06-18-01：项目迁移后后端体检

**阶段约束**

当前进入体验版优化与收口阶段，本轮只做后端与配置体检，不扩展新功能。
...
1. `backend/` 完整存在，包含 `app/`、`migrations/`、`scripts/`、`uploads/`、`.env`、`requirements.txt`、`seed.py`、`tuoban.db` 等核心文件。
2. `backend/scripts/start-dev.cmd` 与 `backend/scripts/start-dev.ps1` 均未引用旧 Obsidian 路径，启动路径通过脚本自身位置计算。
3. `seed.py` 已在新路径下用临时 SQLite 数据库完整跑通，未改动真实业务库。
4. 发现并修复 `seed.py` 一个体验版初始化风险：家长邀请码原先使用随机数，可能在同一个学生的多个家长之间碰撞；现改为按学生 ID + 家长序号生成稳定邀请码。
5. 当前真实存在的数据库是 `backend/tuoban.db`，`.env` 也指向该库；`backend/tuoban_dev.db` 当前不存在。
6. `backend/uploads` 存在，当前可读，照片文件目录正常。
7. `miniprogram/config.js` 当前 API 地址为生产体验版地址：
...
8. `backend/scripts/start-dev.ps1` 默认端口是 `8000`，如果 8000 被占用会自动尝试下一个端口；需要固定 8001 时应显式设置 `PORT=8001`。
9. 本机当前 `8000/8001` 未检测到监听中的本地后端进程，因此浏览器直接访问本地 `/api/health` 需要先启动后端。
10. 通过 FastAPI TestClient 做应用内验证，`GET /api/health` 返回 200，数据库连接和上传目录检查均通过。
11. OpenAPI 已注册照片批量接口：

```text
POST /api/photos/batch
POST /api/photos/batch/associate
```

未登录访问返回 401，说明接口存在且鉴权生效；真实上传验证需要老师登录 token。

**本轮修改文件**

```text
backend/seed.py
Codex-Claude协作联系单-2026-06-09.md
```

**验证命令与结果**

```text
.\.venv\Scripts\python.exe -m compileall app seed.py
通过

FastAPI TestClient GET /api/health
200，通过

FastAPI TestClient 检查 /api/photos/batch 与 /api/photos/batch/associate
已注册；未登录访问返回 401，符合预期
...
```powershell
cd "E:\projects\托班智慧管理系统开发\backend"
.\scripts\start-dev.ps1
```
...
1. 如果要验证本地 `http://127.0.0.1:8001/api/health`，需要先启动或重启本地后端。
2. 需要确认开发库策略：`.env` 指向 `tuoban.db`，但 `start-dev.ps1` 会覆盖为 `sqlite:///./tuoban_dev.db`；目前 `tuoban_dev.db` 不存在。
3. 需要确认小程序体验版是否继续使用 `https://ccrong.cloud/api`。当前不建议自动切回本地 8001，避免影响体验版测试。
4. `scripts/design_inspiration_fetcher.py` 里仍有历史 Obsidian 输出路径，但它不属于后端启动链路；如后续还要使用该设计参考脚本，再单独改。
5. 生产或体验版后端若要使用本轮 `seed.py` 修复，需要按部署流程更新代码；本地修复不会自动影响线上服务。
